...
namespace Deveel.Data {
	...
	public class EntityManager<TEntity> : IDisposable, IAsyncDisposable where TEntity : class {
		...
		/// </summary>
		protected IEntityCache<TEntity>? EntityCache => Services?.GetService<IEntityCache<TEntity>>();
		...
		private async Task SetToCacheAsync(TEntity entity, CancellationToken cancellationToken) {
			...
			try {
				await EntityCache.SetAsync(entity, cancellationToken);
			} catch (Exception ex) {
		...
		private async Task EvictAsync(TEntity entity, CancellationToken cancellationToken) {
			...
			try {
				await EntityCache.RemoveAsync(entity, cancellationToken);
			} catch (Exception ex) {
